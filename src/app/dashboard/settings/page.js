...
export default function SettingsPage() {
  ...
  async function handleSaveProfile(e) {
    e.preventDefault();
    const errors = validateProfile(name, bio, avatarFile);
  ...
  function validateProfile(nextName, nextBio, nextAvatar) {
    const errs = { name: "", bio: "", avatar: "" };
    const trimmed = String(nextName || "").trim();
    if (!trimmed) errs.name = "Name is required";
    else if (trimmed.length < 2)
      errs.name = "Name must be at least 2 characters";

    const bioText = String(nextBio || "");
    if (bioText.length > 180) errs.bio = "Bio must be 180 characters or less";

    const file = nextAvatar || null;
    if (file) {
      const allowed = ["image/png", "image/jpeg", "image/webp"];
      const maxBytes = 2 * 1024 * 1024; // 2MB
      if (!allowed.includes(file.type))
        errs.avatar = "Avatar must be PNG, JPG, or WebP";
      else if (file.size > maxBytes)
        errs.avatar = "Avatar must be 2MB or smaller";
    }
    return errs;
  }
            ...
            >
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setProfileErrors((prev) => ({
                        ...prev,
                        avatar: validateProfile(name, bio, file).avatar,
                      }));
                    }}
                  />
                  <p className="card-text">PNG, JPG, WebP up to 2MB.</p>
                  {profileErrors.avatar ? (
                    <span className="card-text" style={{ color: "crimson" }}>
                      {profileErrors.avatar}
                    </span>
                  ) : null}
                </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setProfileErrors((prev) => ({
                      ...prev,
                      name: validateProfile(next, bio, avatarFile).name,
                    }));
                  }}
                  required
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    ...
                    setProfileErrors((prev) => ({
                      ...prev,
                      bio: validateProfile(name, next, avatarFile).bio,
                    }));
                  }}
                />
                <div
              ...
              <div className="card-actions" style={{ display: "flex", gap: 8 }}>
                <Button
                  type="submit"
                  disabled={
                    saving || Object.values(profileErrors).some(Boolean)
                  }
                >
